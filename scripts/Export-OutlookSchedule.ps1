...
function Get-ComBool {
    param([object]$Object, [string]$PropertyName, [bool]$DefaultValue = $false)
    try {
        $value = $Object.$PropertyName
        if ($null -eq $value) { return $DefaultValue }
        return [bool]$value
...
function Get-CalendarTargets {
    ...
    foreach ($entry in @($shared)) {
        ...
        if ($entry -is [string]) {
            ...
            if ($null -ne $entry.PSObject.Properties["Email"]) { $email = [string]$entry.Email }
...
try {
    ...
    $jsonSafe = ConvertTo-SafeJson -Rows $sortedRows
